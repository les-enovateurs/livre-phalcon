...
$di->setShared('dispatcher', function () {
    ...
    $oGestionEvenement->attach(
        'dispatch',
        function (Event $oEvent, $oDispatcher) use ($oProfiler, $oLogger) {
            if ($oEvent->getType() === 'beforeDispatch') {
                $oProfiler->startProfile($oDispatcher->getControllerName().'/'.$oDispatcher->getActionName());
            }
    
            if ($oEvent->getType() === 'afterDispatch') {
                $oProfiler->stopProfile();

                $oResultat = $oProfiler->getLastProfile();
                $oLogger->debug('Appel du contrôleur : '. $oDispatcher->getControllerName());
                $oLogger->debug('Appel de l\'action : '. $oDispatcher->getActionName());
