...
class BddController extends ControllerBase
{
    ...
    public function transactionAction(){
        ...
        $this->view->avant_transaction = $this->db->fetchAll($sSql, Db::FETCH_OBJ);
        try {
            // Démarrage de la transaction
            $this->db->begin();
        
            $this->db->execute('DELETE FROM utilisateurs WHERE id = 1');
            $this->view->apres_suppression = $this->db->fetchAll($sSql, Db::FETCH_OBJ);
            ...
            $this->view->apres_rollback = $this->db->fetchAll($sSql, Db::FETCH_OBJ);
            
        }

        /** SANS ERREUR */
        $sSql = 'SELECT * FROM utilisateurs ORDER BY id ASC LIMIT 2';
        $this->view->avant_transaction_sans_erreur = $this->db->fetchAll($sSql, Db::FETCH_OBJ);
        try {
            // Démarrage de la transaction
            $this->db->begin();
        
            $this->db->execute('DELETE FROM utilisateurs WHERE id = 1');
            $this->view->apres_suppression_sans_erreur = $this->db->fetchAll($sSql, Db::FETCH_OBJ);

            $this->db->execute("UPDATE utilisateurs SET nom='ANCIEN' WHERE id = 2");
        
            $this->view->apres_modification_sans_erreur = $this->db->fetchAll($sSql, Db::FETCH_OBJ);

            // Fin de la transaction et application des requêtes
            $this->db->commit();

            $this->view->apres_suppression_apres_commit = $this->db->fetchAll($sSql, Db::FETCH_OBJ);
